    ...
    def test_surface_policy_moves_back_to_object(self) -> None:
        ...
        with MontyObjectRecognitionExperiment(config) as exp:
            ...
            for loader_step, observation in enumerate(exp.dataloader):
                ...
                # When the Action is ObjectNotVisible and the Source is touch_object
                # it means that we are observing the short-circuiting behavior from the
                # DataLoader. If the policy moves such that the object is not visible,
                # the DataLoader executes the TouchObject positioning procedure. After
                # the procedure completes, the DataLoader returns a motor-only step
                # observation arrived at by the positioning procedure.
                #  Step | Action           | Motor-only? | Obs processed? | Source
                # ------|------------------|-------------|----------------|-------------
                #  0    | None             | True        | False          | first_step
                #  1    | ObjectNotVisible | True        | False          | touch_object
                ...
                # falls off object but positioning procedure returns back to object
                #  13   | ObjectNotVisible | True        | False          | touch_object
                ...
                # falls off object but positioning procedure returns back to object
                #  18   | ObjectNotVisible | True        | False          | touch_object
                ...
                # falls off object but positioning procedure returns back to object
                #  23   | ObjectNotVisible | True        | False          | touch_object
                ...
                # falls off object but positioning procedure returns back to object
                #  28   | ObjectNotVisible | True        | False          | touch_object

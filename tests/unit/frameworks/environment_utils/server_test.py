...
class MontyRequestHandlerTest(unittest.TestCase):
    def test_monty_request_handler_put_depth(self):
        ...
        with inert_server as server, monkey_patcher() as mp, TempDir() as temp_dir:
            ...
            # Confirm blank slate
            self.assertEqual(len(list(data_dir.glob("depth_*.data"))), 0)
            ...
            self.assertEqual(result.status_code, 201)
            self.assertEqual(result.text, 'Saved "depth_1.data"\n')

            # Clean up server thread
            server_thread.join()

            # Check that the files were created correctly
            self.assertEqual(len(list(data_dir.glob("depth_*.data"))), 2)

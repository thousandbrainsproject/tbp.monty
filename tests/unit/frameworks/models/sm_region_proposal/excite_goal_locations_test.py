...
from tbp.monty.frameworks.models.sm_region_proposal.excite_goal_locations import (
    ExciteGoalLocations,
)
...
def context_with_goals(*goals: MagicMock) -> MagicMock:
    # The proposer reads only the goals and the module id off the context.
    context = MagicMock(spec=SMRegionContext)
    context.goals = goals
    context.sensor_module_id = "patch_0"
    return context
...
class ExciteGoalLocationsTest(unittest.TestCase):
    ...
    def test_names_the_module_as_sender(self) -> None:
        region = self.proposer(context_with_goals(goal_at(np.zeros(3))))
    ...
    def test_proposes_one_ball_per_located_goal(self) -> None:
        ...
        region = self.proposer(
            context_with_goals(goal_at(near), goal_at(None), goal_at(far))
        )

...
Briefly, the most significant changes proposed here are as follows:
 ...
 2. Goal states generated by learning modules and sensor modules with be sent to a `GoalStateSelector`, a new Monty component.
 3. The role of the `GoalStateSelector` is to produce a single output goal state from any number of goal states.
 4. Motor systems will be responsible for selecting a policy appropriate to for the goal state.
 
# Implementation

![Information Flow](efficient_saccades/architecture_overview.png)
**Proposed Routing Pathways**. LM- and SM-derived goal states will be routed to the  `GoalStateSelector`, and the `GoalStateSelector`'s output will be routed to the motor system.

The `GoalStateSelector` will receive any number of goal states derived from learning and sensor modules, and it will produce at most one goal state for consumption by the motor system. There are many potential strategies for choosing a goal state, but the simplest and first approach will be to select the goal state with the largest `confidence` value. In this way, it will act exactly as the `Monty` class currently does. More complex methods are outside of the scope of this RFC.
...
### `SensorModule`
 ...
 - Will have a `propose_goal_state` method that returns any number of goal states. Note that this differs from a learning module's `propose_goal_state` which returns at most 1 goal state. Returing many goal states will enable planned work on salience-based targeting methods.

All of the above changes are meant to mirror existing learning-module/GSG behavior.

### `Monty`
 - Will have a `gss` attribute to hold a `GoalStateGenerator`.
...
Specific modifications to `GoalState` objects are not yet understood. However, we expect that additional metadata may be required for the motor system to make correct judgements about which policy to select. For example, implementing behavior like `GetGoodView` will require some way to distinguish between goal states that specify an
agent's translation vs goal states specifying an orienting motion.

# Long-Term Objectives

Following discussions on this RFC, we have identified three objectives coupled to the functional and architectural changes proposed here.

  1. Implement the functionality of `GetGoodView` for the distant agent within the proposed framework. Goal-state-generating SMs will provide the model-free signals used to orient the agent to on-object regions. These signals can be filtered out or overridden when learning modules emit goal states, and the filtering with be performed by a `GoalStateSelector`.
  2. Integration of model-free segmentation and model-based policies to recognize an object before moving onto a new object. This objective is the primary motive that led to the formation of this specific RFC.
...
Each of these tasks is significant, likely warranting their own RFCs. Further discussion is outside the scope of this RFC.

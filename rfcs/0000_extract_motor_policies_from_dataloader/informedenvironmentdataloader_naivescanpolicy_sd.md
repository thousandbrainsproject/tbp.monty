...
sequenceDiagram
    ...
    EDLO ->> NSP : self.state = state
    EDLO ->>+ NSP : agent_id
    NSP -->>- EDLO : agent_id
    EDLO ->> NSP : self.state[agent_id]["motor_only_step"] = False
    deactivate EDLO
    EDLO -->>- IEDL : ...
    IEDL ->>+ DS : env
    DS -->>- IEDL : env
        ...
        NOTE right of IEDL : Bookmark get_good_view_with_patch_refinement
            ...
            NOTE right of IEDL : Bookmark get_good_view
                opt multiple_objects_present
                    IEDL ->>+ NSP : is_on_target_object
                    NSP -->>- IEDL : on_target_object
                    opt not on_target_object
                        IEDL ->>+ NSP : orient_to_object
                        NSP -->>- IEDL : actions
                        loop action in actions
                            IEDL ->>+ DS : __getitem__(action)
                            DS -->>- IEDL : observation, state
                            IEDL ->> IEDL : self._observation = observation
                            IEDL ->> NSP : self.state = state
                        end
                    end
                end
                opt allow_translation
                    IEDL ->>+ NSP : move_close_enough
                    NSP -->>- IEDL : action, close_enough
    ...
    opt self._counter == 0
        IEDL ->>+ IEDL : first_step
            IEDL ->>+ NSP : agent_id
            NSP -->>- IEDL : agent_id
            IEDL ->> NSP : self.state[agent_id]["motor_only_step"] = False
        deactivate IEDL
        break early return
            IEDL -->> E : self._observation
        end
    end
    IEDL ->>+ NSP : use_goal_state_driven_actions
    NSP -->>- IEDL : use_goal_state_driven_actions
    IEDL ->>+ NSP : driving_goal_state
    NSP -->>- IEDL : driving_goal_state
    opt use_goal_state_driven_actions and driving_goal_state is not None
        IEDL ->>+ IEDL : execute_jump_attempt
            IEDL ->>+ NSP : agent_id
            NSP -->>- IEDL : agent_id
            IEDL ->>+ NSP : self.state[agend_id]
            NSP -->>- IEDL : pre_jump_state
            IEDL ->>+ NSP : derive_habitat_goal_state
            NSP -->>- IEDL : target_loc, target_np_quat
            ...
            IEDL ->> IEDL : self._observation = observation
            IEDL ->> NSP : self.state = state
            IEDL ->>+ NSP : get_depth_at_center
            NSP -->>- IEDL : depth_at_center
            alt depth_at_center < 1.0
                ...
                IEDL ->>+ IEDL : handle_failed_jump
                    ...
                    IEDL ->>+ NSP : agent_id
                    NSP -->>- IEDL : agent_id
                    IEDL ->>+ NSP : self.state[agent_id]["position"]
                    NSP -->>- IEDL : position
                    IEDL ->>+ NSP : self.state[agent_id]["rotation"]
                    NSP -->>- IEDL : rotation
                    IEDL ->>+ NSP : self.state[agent_id]["sensors"]
                    NSP -->>- IEDL : sensors
                deactivate IEDL
            end
            IEDL ->>+ NSP : agent_id
            NSP -->>- IEDL : agent_id
            IEDL ->> NSP : self.state[agent_id]["motor_only_step"] = True
            IEDL ->>+ NSP : action
            NSP -->>- IEDL : action
            IEDL ->>+ NSP : post_action(action)
            NSP -->>- IEDL : ...
        deactivate IEDL
        break early return
            IEDL -->> E : self._observation

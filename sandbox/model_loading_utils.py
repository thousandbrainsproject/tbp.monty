...
def load_object_model(model_path, object_name, lm_id=0):
    ...
    patch_keys = [key for key in object_data if key.startswith("patch")]
    if len(patch_keys) != 1:
        raise RuntimeError(
            f"Expected exactly one patch key, got {patch_keys}. "
            "Specify the correct patch key manually."
        )
    graph_data = object_data[patch_keys[0]]._graph

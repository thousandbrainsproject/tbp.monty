"""Script to compare three edge detection methods on synthetic RGB images.

Reads RGB PNG images from synthetic_edge_test_images directory (including subdirectories),
applies three edge detection methods, and creates comparison visualizations with results saved to CSV.
"""
...
def main():
    ...
    output_dir = Path("synthetic_edge_test_images_comparison")
    output_dir.mkdir(parents=True, exist_ok=True)

    # Find all PNG files recursively
    image_files = sorted(input_dir.rglob("*.png"))
    ...
    for image_file in tqdm(image_files, desc="Processing images"):
        try:
            ...
            subdirectory = str(relative_path.parent) if relative_path.parent != Path(".") else ""

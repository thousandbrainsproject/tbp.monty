...
class InformedEnvironmentDataLoader(EnvironmentDataLoaderPerObject):
    ...
    def get_good_view(
        ...
        beginning of an episode with respect to a given sensor. Also currently used
        by the distant agent after a "jump" has been initialized by a model-based
        policy.

        First, the agent is moved towards the target object using actions generated by
        the motor system's `move_close_enough` method. This step is performed by
        default but can be skipped by setting `allow_translation=False`. Second, the
        agent will then be oriented towards the object so that the sensor's central
        pixel is on-object.

        In the case of multi-object experiments, (i.e., when `num_distractors > 0`),
        there is an additional orientation step performed prior to the translational
        movement step.

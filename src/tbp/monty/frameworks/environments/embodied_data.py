...
class InformedEnvironmentDataLoader(EnvironmentDataLoaderPerObject):
    ...
    def get_good_view(self, view_sensor_id: str, allow_translation: bool = True):
        ...
        beginning of an episode - the surface agent makes use of the
        touch_object method instead. Also currently used by the distant agent
        after a "jump" has been initialized by a model-based policy.

        This is typically called twice in a row. First, it is called with the view
        finder's `view_sensor_id` and `allow_translation=True`. In this case, the
        agent will move towards the object using actions generated by the motor
        system's `move_close_enough` method which steps forward until the object
        fills a certain percentage of the view finder's image (specified by the motor
        system's `good_view_percentage` attribute) or until the agent reaches a certain
        distance from the object (specified by the motor system's
        `desired_object_distance` attribute). This makes sure that big and small objects
        all fill similar amount of space in the sensor's field of view. The agent
        will then be oriented towards the object so that the view finder's central
        pixel is on-object if necessary. `get_good_view` is then called again with the
        `view_sensor_id` corresponding to the central sensor patch (typically "patch"
        or "patch_0") and `allow_translation=False`. In this case, the agent will
        use the patch to perform finer orientation movements to ensure that the
        patch's central pixel is on-object.

        Finally, this function has slightly different behavior for multi-object
        experiments (i.e., when `num_distractors > 0`). In this case, the agent is
        first oriented towards the object, then (optionally) moves towards it, and
        finally performs a final reorientation if necessary.

        Note that translational or rotational actions are only performed if the
        criteria for the respective action are not met.

...
class TargetFindingGsg(SmGoalStateGenerator):
    ...
    def _generate_output_goal_state(
        ...
        # Naive sort by x-coordinate.
        goal_states.sort(key=lambda x: x.location[0])
        confidences = np.linspace(1.0, 0.5, len(goal_states))
        for i, g in enumerate(goal_states):
            g.confidence = confidences[i]
...
@dataclass
class Target:
    center_pix: np.ndarray | None = None
    size_pix: int | None = None
...
class TargetFinder:
    ...
    def __call__(self, image: np.ndarray):
        ...
        for i in range(1, num_features + 1):
            ...
            targets.append(
                Target(
                    center_pix=np.array([row, col], dtype=int),

...
class FixmeAbstractEvidenceGraphLM(GraphLM, EvidenceUpdaterProtocol):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(FixmeAbstractEvidenceGraphLM, self).__init__(*args, **kwargs)
        # --- LM components ---
    ...
    def _update_possible_matches(self, query):
        ...
        for graph_id in self.get_all_known_object_ids():
            if self.use_multithreading:
                ...
                t = threading.Thread(
                    target=self.update_evidence,
                    args=(query[0], query[1], graph_id),
                )
                thread_list.append(t)
            else:  # This can be useful for debugging.
                self.update_evidence(query[0], query[1], graph_id)
        if self.use_multithreading:
...
class EvidenceGraphLM(FixmeAbstractEvidenceGraphLM, DefaultEvidenceUpdater):
    pass

...
The default approach initializes the full hypothesis space once at the beginning of an episode and updates evidence for all hypotheses at every step. This works well when the agent observes a single object throughout the episode, but it cannot adapt to changes in the environment, for example, when the agent moves from one object to another in an environment, or from one child object to another within a compositional model. In these cases the existing hypotheses, which were sampled for the previous object, no longer match the incoming observations.
...
The burst sampling hypotheses updater is a step toward this goal. It dynamically adds and removes hypotheses based on how well they explain the incoming observations. When the agent transitions to a new object, the evidence slopes of the old hypotheses drop, triggering a burst that samples new hypotheses informed by the new observations. The outdated hypotheses are quickly deleted as they cease to accumulate evidence, while some of the newly sampled hypotheses rapidly accumulate evidence for the current object.

...
Predictive coding is a proposal for how a hierarchical system can use prediction errors to develop models of the world. Similar to predictive coding, **Monty relies on prediction errors for inferring what it is sensing**. Beliefs in Monty about what objects it is perceiving, as well as their spatial details, are are formed using a mismatch (i.e., an error) between predictions made by Monty's learned models, and sensory observations. However, in contrast to predictive coding, **information passed further in the hierarchy in the TBT is a representation of what the learning module infers, rather than the remaining "prediction error".** In this way, models are formed as an arrangement of features, which in turn represent other models. In addition to this core distinction, it's worth addressing that predictive coding is often put forward as a biologically plausible mechanism for deep learning in the brain. However, beyond questions about biological plausibility, this approach does not circumvent the various drawbacks present in deep learning systems, such as poor sample efficiency and catastrophic forgetting.

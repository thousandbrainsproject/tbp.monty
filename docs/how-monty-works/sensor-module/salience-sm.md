...
## SalienceStrategy
`SalienceSM` uses a `SalienceStrategy` to compute salience maps. The `SalienceStrategy` is a configurable component of `SalienceSM`. There are currently two `SalienceStrategy` implementation in Monty:

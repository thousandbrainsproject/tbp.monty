...
def load_object_model(
    ...
    if "dist" in exp_name:
        pretrain_dir = pretrain_dir / "supervised_pre_training_all_objects" / "pretrained"
    elif "surf" in exp_name:
        pretrain_dir = pretrain_dir / "supervised_pre_training_surface_view" / "pretrained"
    else:
        raise ValueError(f"The experiment name must contain 'dist' or 'surf' in order to load correct pretrained model for objects.")
...
class TargetHypothesisVisualizer:
    ...
    def __init__(self, json_path: str, model_name: str = "dist_agent_1lm", timestep: int = 25):
    ...
    def load_episode_data(self) -> None:
        ...
        self.target_object_name = self.target_data.get("primary_target_object", self.target_data.get("object", ""))
        ...
        if self.timestep >= len(self.lm_data["possible_locations"]):
            print(f"Warning: timestep {self.timestep} not available, using last timestep")
        ...
        if self.target_object_name not in timestep_data:
            raise ValueError(f"No {self.target_object_name} data found at timestep {self.timestep}")
        
        self.target_locations = np.array(timestep_data[self.target_object_name])
        self.target_evidences = np.array(self.lm_data["evidences"][self.timestep][self.target_object_name])
        
        print(f"Loaded {len(self.target_locations)} {self.target_object_name} hypotheses from timestep {self.timestep}")
        print(f"Evidence range: [{self.target_evidences.min():.4f}, {self.target_evidences.max():.4f}]")
        
        # Get target object pose
        self.target_position = np.array(self.target_data.get("primary_target_position", self.target_data.get("position", [0, 0, 0])))
        self.target_rotation = np.array(self.target_data.get("primary_target_rotation_euler", self.target_data.get("euler_rotation", [0, 0, 0])))
        ...
        if "current_mlh" in self.lm_data and self.timestep < len(self.lm_data["current_mlh"]):
    ...
    def load_target_model(self) -> None:
        ...
        try:
            ...
            print(f"Loaded {self.target_object_name} model with {len(self.target_model.pos)} points")
    ...
    def create_visualization(self) -> None:
        ...
        plotter = Plotter(title=f"Sensor Location Hypotheses for {self.target_object_name.title()} - Episode 0, Timestep {self.timestep}")
        ...
        if self.target_model is not None:
            ...
            # Add label
            plotter.add(Text2D(f"{self.target_object_name.title()} Object (Ground Truth)", pos="top-left", s=1))
        ...
        evidence_shifted = self.target_evidences - self.target_evidences.min() + 1.0  # Shift to positive
        evidence_log = np.log(evidence_shifted)
        evidence_norm = (evidence_log - evidence_log.min()) / \
                       (evidence_log.max() - evidence_log.min() + 1e-8)
        
        print(f"Evidence normalization - Min: {evidence_norm.min():.3f}, Max: {evidence_norm.max():.3f}")
